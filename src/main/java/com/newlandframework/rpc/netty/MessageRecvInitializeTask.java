 ...
 * Rpc服务器消息线程任务处理
 * 
 * 根据RPC消息的请求报文，利用反射得到最终的计算结果，并把结果写入RPC应答报文结构
 */
public class MessageRecvInitializeTask extends AbstractMessageRecvInitializeTask {
	...
	private AtomicReference<InvokeEventWatcher> watcher = new AtomicReference<InvokeEventWatcher>(
	...
	protected void injectInvoke() {
		...
		try {
			Method method = ReflectionUtils.getDeclaredMethod(cls, request.getMethodName(),
					request.getTypeParameters());
			...
			facade.set(new InvokeEventBusFacade(ModuleMetricsHandler.getInstance(), visitor.get().getModuleName(),
					visitor.get().getMethodName()));
